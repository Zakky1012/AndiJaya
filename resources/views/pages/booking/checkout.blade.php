                ...
                <h2 class="font-bold text-xl leading-[30px]">Your Booking</h2>
                    <div class="flex justify-between">
                        <div>
                            ...
                            <p class="font-semibold text-lg">
                                {{ $keberangkatan->segments->first()->destinasi->kota }} (
                                {{ $keberangkatan->segments->first()->destinasi->iata_code }}
                                )
                            </p>
                        </div>
                        <div class="text-end">
                            ...
                            <p class="font-semibold text-lg">
                                {{ $keberangkatan->segments->last()->destinasi->kota }} (
                                {{ $keberangkatan->segments->last()->destinasi->iata_code }}
                                )
                            </p>
                        </div>
                    </div>
                    <div class="flex justify-between">
                        <div>
                            ...
                            <p class="font-semibold text-lg">
                                {{ $keberangkatan->segments->first()->time->format('d F Y')}}
                            </p>
                        </div>
                        <div class="text-end">
                                ...
                                <div class="flex items-center gap-[10px]">
                                    <div>
                                        ...
                                        <p class="text-sm text-garuda-grey mt-[2px]">
                                            {{ $keberangkatan->segments->first()->time->format('H:i') }} -
                                            {{ $keberangkatan->segments->last()->time->format('H:i') }}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex flex-col gap-[2px] items-center justify-center">
                                    <p class="text-sm text-garuda-grey">
                                        {{ number_format($keberangkatan->segments->first()->time->diffInHours($keberangkatan->segments->last()->time), 0) }}
                                        hours
                                    </p>
                                    <div class="flex items-center gap-[6px]">
                                        ...
                                        @if ($keberangkatan->segments->count() > 2)
                                    ...
                                    @if ($keberangkatan->segments->count() > 2)
                                        <p class="text-sm text-garuda-grey">Transit {{ $keberangkatan->segments->count() - 2}}</p>
                                    @else
                                        <p class="text-sm text-garuda-grey">direct</p>
                                    @endif
                                </div>
                                <p class="font-semibold text-garuda-green text-center">
                                    {{ 'Rp. ' . number_format($keberangkatan->classes->first()->harga, 0, ',', '.') }}
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex justify-between">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-lg leading-[27px] mt-[2px]">
                                    {{ \Str::ucfirst($tier->tipe_kelas) }}
                                </p>
                            </div>
                            <div>
                                <p class="text-sm text-garuda-grey">Seats</p>
                                <p class="font-semibold text-lg leading-[27px] mt-[2px]">
                                    {{ implode(', ', $keberangkatan->seats->whereIn('id', $transaction['selected_seats'])->pluck('name')->toArray()) }}
                                </p>
                            </div>
                        </div>
                    ...
                    <div id="Passenger-{{ $loop->index + 1 }}" class="accordion-with-select group flex flex-col h-fit rounded-[20px] bg-white overflow-hidden transition-all duration-300">
                        ...
                        <div class="accordion-content p-5 pt-0 flex flex-col gap-5">
                            <label class="flex flex-col gap-[10px]">
                                ...
                                @error('passengers.' . $loop->index . '.nama') border-red-500 @enderror">
                                    <img src="{{ asset('assets/images/icons/profile-black.svg') }}" class="w-5 flex shrink-0" alt="icon">
                                    <input type="text" id="" class="appearance-none outline-none w-full font-semibold placeholder:font-normal" placeholder="Write your complete name" value="{{ $passenger['nama'] }}">
                                </div>
                        ...
                        <div class="flex flex-col gap-[10px]">
                            ...
                            <p class="font-semibold">Date of Birth</p>
                                <input type="hidden" id="dateOfBirth-{{ $loop->index }}" data-index="{{ $loop->index }}">
                            ...
                            <div class="flex items-center gap-[10px]">
                                ...
                                @error('passengers.' . $loop->index . '.date_of_birth') border-red-500 @enderror>
                                    <img src="{{ asset('assets/images/icons/note-add-black.svg') }}"
                                        ...
                                        alt="icon">
                                    <select id="day-select-{{ $loop->index }}" class="date-select day-select appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal"
                                ...
                                @error('passengers.' . $loop->index . '.date_of_birth') border-red-500 @enderror>
                                    <img src="{{ asset('assets/images/icons/note-add-black.svg') }}"
                                        class="absolute transform -translate-y-1/2 top-1/2 left-5 w-5 shrink-0"
                                        alt="icon">
                                    <select id="month-select-{{ $loop->index }}" class="date-select month-select appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal"
                                        data-index="{{ $loop->index }}"
                                        onchange="updateDateOfBirth({{ $loop->index }})">
                                        @for ($i = 1; $i <= 12; $i++)
                                            <option value="{{ $i }}"
                            ...
                            <label class="flex flex-col gap-[10px]">
                                <p class="font-semibold">Kewarganegaraan</p>
                                ...
                                @error('passengers.' . $loop->index . '.kewarganegaraan') border-red-500 @enderror>
                                    ...
                                    <select id="" class="appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal">
                                        <option hidden>Select country region</option>
                                        <option value="Singapore"
                                            {{ $passenger['kewarganegaraan'] === 'Singapore' ? 'selected' : '' }}>Singapore
                                        </option>
                                        <option {{ $passenger['kewarganegaraan'] === 'Japan' ? 'selected' : '' }}>Japan
                                        </option>
                                        <option {{ $passenger['kewarganegaraan'] === 'Indonesia' ? 'selected' : '' }}>Indonesia
                                        </option>
                                    </select>
                                </div>
                                @error('passengers.' . $loop->index . '.kewarganegaraan')
                                    <p class="text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </label>
                    ...
                    <div class="flex flex-col gap-[10px]">
                        ...
                        <div class="flex items-center flex-nowrap gap-[10px]">
                            <label
                                ...
                                <input type="radio" class="absolute opacity-0 left-1/2" required>
                            </label>
                        </div>
                    </div>
                </div>
...
<script>
    window.addEventListener('promoCodeUpdated', event => {
        ...
        const harga              = parseFloat('{{ $tier->harga }}');
        const quantity           = parseInt('{{ count($transaction['selected_seats']) }}');
        const totalWithoutDiscount = (harga * quantity) * 1.11;

        // variable untuk menyimpan total baru dan total diskon
        let newTotal;
        let totalPromo = 0;
        ...
        if (discountType === 'percentage') { // Perbaikan: Gunakan '===' untuk perbandingan
            totalPromo = (harga * quantity) * (discountValue / 100);
        } else {
            totalPromo = discountValue;
        }

        // Perbaikan: Pastikan total dihitung dengan benar dari subtotal, diskon, dan pajak
        const subTotal = harga * quantity;
        const totalTax = subTotal * 0.11;
        newTotal = (subTotal - totalPromo) + totalTax;

        // Tampilkan hasil perhitungan dan promo yang diterapkan
        // Perbaikan: Gunakan innerHTML, bukan innetHtml
